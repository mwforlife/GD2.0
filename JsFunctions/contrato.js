...
function validarform1() {
  ...
  if (regioncelebracion <= 0) {
    ...
    $("#numero").focus();
    return false;
  } else if (ruttrabajador.trim().length == 0) {
    ToastifyError("Debe ingresar el rut del trabajador");
    mostrar1();
    $("#ruttrabajador").focus();
    return false;
  } else if (nombretrabajador.trim().length == 0) {
    ToastifyError("Debe ingresar el nombre del trabajador");
    mostrar1();
    $("#nombretrabajador").focus();
    return false;
  } else if (apellidos.trim().length == 0) {
    ToastifyError("Debe ingresar los apellidos del trabajador");
    mostrar1();
    $("#apellidos").focus();
    return false;
  } else if (fechanacimiento == false) {
    ToastifyError("Debe seleccionar la fecha de nacimiento");
    mostrar1();
    $("#fechanacimiento").focus();
    return false;
  } else if (sexo <= 0) {
    ToastifyError("Debe seleccionar el sexo");
    mostrar1();
    return false;
  } else if (nacionalidad.trim().length == 0) {
    ToastifyError("Debe ingresar la nacionalidad");
    mostrar1();
    $("#nacionalidad").focus();
    return false;
  } else if (correotrabajador.trim().length == 0) {
    ToastifyError("Debe ingresar el correo electronico");
    mostrar1();
    $("#correotrabajador").focus();
    return false;
  } else if (telefonotrabajador.trim().length == 0) {
    ToastifyError("Debe ingresar el telefono");
    mostrar1();
    $("#telefonotrabajador").focus();
    return false;
  } else if (trabajadorregion <= 0) {
    ToastifyError("Debe seleccionar la region");
    mostrar2();
    $("#trabajadorregion").focus();
    return false;
  } else if (trabajadorcomuna <= 0) {
    ToastifyError("Debe seleccionar la comuna");
    mostrar1();
    $("#trabajadorcomuna").focus();
    return false;
  } else if (calletrabajador.trim().length == 0) {
    ToastifyError("Debe ingresar la calle");
    mostrar1();
    $("#calletrabajador").focus();
    return false;
  } else if (numerotrabajador.trim().length == 0) {
    ToastifyError("Debe ingresar el numero");
    mostrar1();
...
function validarform3() {
  if (validarform1() == true && validarform2() == true) {
    ...
    if (tiposueldo <= 0) {
      ...
      ToastifyError("Debe ingresar la AFP");
      mostrar3();
      $("#afp").focus();
      return false;
    } else if (salud <= 0) {
      ToastifyError("Debe ingresar la salud");
      mostrar3();
      $("#salud").focus();
      return false;
      //Validar el otro tipo de pacto esta checkeado
...
function generar(valor) {
  ...
  if ($("#juramento").is(":checked")) {
    $.ajax({
      url: "php/pdf/generarcontratoindividual1.php",
      type: "POST",
      data: {
      ...
      success: function (data) {
        ...
        if (data == 1 || data == "1") {
          ToastifySuccess("Contrato Registrado Con Exito");
          setTimeout(function () {
            window.location.href = "buscartrabajador.php";
          }, 2000);
        } else {
          ToastifyError(data);
        }
