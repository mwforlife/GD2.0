...
function gettrabajadores() {
  ...
  for (var i = 0; i < trabajadores.length; i++) {
    ...
    html +=
      '<td><button type="button" class="btn btn-danger" onclick="eliminatrabajador(' +
      trabajadores[i].id +
...
function procesartodo() {
  ...
  if (validalista()) {
    ...
    //Capturar el valor del atributo termino del elemento del select
    var termino =
      element.options[element.selectedIndex].getAttribute("termino");
    //Capturar la Entidad del elemento del select
    var entidad =
      element.options[element.selectedIndex].getAttribute("entidad");
      ...
      success: function (response) {
        try {
          ...
          if (json.status == true) {
            ...
            if (html.trim().length > 0 && htmlerror.trim().length == 0) {
              setTimeout(function () {
