...
if (isset($_POST['liquidaciones'])) {
	...
	$liquidaciones = json_decode($liquidaciones);
	...
	if ($result == true) {
		echo json_encode(array('status' => true, 'message' => 'Liquidaciones eliminadas correctamente'));
	} else {
		echo json_encode(array('status' => false, 'message' => 'Error al eliminar las liquidaciones'));
	}
} else {
