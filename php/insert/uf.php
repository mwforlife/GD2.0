...
if (isset($_POST['periodo']) && isset($_POST['tasa'])) {
    ...
    if (strlen($periodo) == 0 || strlen($tasa) == 0) {
        ...
        $periodo = $periodo . "-01";
        $result = $c->registraruf($periodo, $tasa);
        if ($result == true) {
            echo json_encode(array('status' => true, 'message' => 'UF registrada correctamente'));
        } else {
            echo json_encode(array('status' => false, 'message' => 'Error al registrar UF'));
        }
    }
    ...
    $periodo = $mes . " " . $anio;
    $eventos = "Se registro la UF del Periodo " . $periodo . " con la tasa: " . $tasa;
