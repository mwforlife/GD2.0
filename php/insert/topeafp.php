...
if (isset($_POST['periodo']) && isset($_POST['tope'])) {
    ...
    if (strlen($periodo) == 0 || strlen($tope) == 0) {
        ...
        $periodo = $periodo . "-01";
        $result = $c->registrartopeafp($periodo, $tope);
        if ($result == true) {
            echo json_encode(array('status' => true, 'message' => 'TOPE AFP registrada correctamente'));
        } else {
            echo json_encode(array('status' => false, 'message' => 'Error al registrar TOPE AFP'));
        }
    }
    ...
    $periodo = $mes . " " . $anio;
    $eventos = "Se registro la TOPE AFP del Periodo " . $periodo . " con la tope: " . $tope;
