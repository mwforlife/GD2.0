<?php
/**
 * Registrar Contratos Masivos Express - Sin documento PDF
 * Este archivo registra contratos para multiples trabajadores del lote sin generar PDF
 */
...
try {
    ...
    if ($empresa <= 0) {
        throw new Exception("ID de empresa no valido");
    }
    ...
    if (count($lote) == 0) {
        throw new Exception("No hay trabajadores en el lote. Por favor, seleccione trabajadores primero.");
    }
    ...
    foreach ($lote as $trabajador) {
        try {
            $trabajadorId = $trabajador->getEmpresa();
            $tra = $c->buscartrabajador($trabajadorId);

            if (!$tra) {
                $errores[] = "Trabajador ID {$trabajadorId} no encontrado";
                continue;
            }
            ...
            // SQL INSERT para cada trabajador (sin campos de AFP, Salud, banco)
            ...
            // Debug: guardar SQL para revision
            error_log("SQL Contrato Masivo Express: " . $sql);

            $contratoId = $c->query_id($sql);

            if (!$contratoId || $contratoId <= 0) {
                $errorSQL = $c->getLastError();
                error_log("Error MySQL Contrato Masivo Express: " . $errorSQL);
                $errores[] = "Error al registrar contrato para " . $tra->getNombre() . " " . $tra->getApellido1();
                continue;
            }
            ...
            $eventos = "Registro contrato express masivo para el trabajador " .
            ...
            $errores[] = "Error con trabajador " . ($tra ? $tra->getNombre() . " " . $tra->getApellido1() : "ID: " . $trabajadorId) . ": " . $e->getMessage();
            error_log("Error contrato masivo express trabajador {$trabajadorId}: " . $e->getMessage());
        }
    }
    ...
    if (count($contratosRegistrados) > 0) {
        $centroCostoObj = $c->buscarcentrcosto($centrocosto);
        $centroCostoNombre = $centroCostoObj ? $centroCostoObj->getNombre() : "SIN_CC";

        $fecha = date('Ymdhis');
        $nomlote = "TRABAJADORES " . $centroCostoNombre . "_" . $fecha;

        // Insertar el lote y obtener ID directamente (evita condicion de carrera)
        ...
        if ($loteId && $loteId > 0) {
            ...
            $usuario = $_SESSION['USER_ID'];
            $eventos = "Registro lote de contratos masivos express '$nomlote' con " . count($contratosRegistrados) . " contratos";
            $c->RegistrarAuditoriaEventos($usuario, $eventos);
        }
    }
    ...
    if (count($errores) > 0 && count($contratosRegistrados) == 0) {
        ...
        echo json_encode([
            ...
            'lote_id' => $loteId,
            'lote_nombre' => $nomlote,
            'message' => 'Contratos express registrados exitosamente'
        ]);
    }

} catch (Exception $e) {
    // Registrar error en log
    error_log("Error al registrar contratos masivos express: " . $e->getMessage());
