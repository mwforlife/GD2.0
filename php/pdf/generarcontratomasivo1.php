...
if (isset($_POST['idempresa'])  && isset($_POST['tipocontratoid'])) {
    ...
    $nomlote = $centrocosto . "_" . $fecha;
    $c->query("insert into lotes values(null,'$nomlote',$empresa,now());");
    $id = $c->buscaridultimolote();
    //Recorrer el array de ids de contratos y se guardan en la tabla lotes_contratos
    foreach ($idscontrato as $idcontrato) {
        $valid = $c->validarloteids($id, $idcontrato);
        if ($valid == false) {
            $c->query("insert into detallelotes values(null,$idcontrato,$id,now())");
        }
    }
    echo 1;
}
