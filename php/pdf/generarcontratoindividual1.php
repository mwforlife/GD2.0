...
if (isset($_POST['idempresa']) && isset($_POST['idtrabajador']) && isset($_POST['tipocontratoid'])) {
    ...
    /*categoria_contrato: categoria_contrato, regioncelebracion:regioncelebracion,comunacelebracion:comunacelebracion,fechacelebracion:fechacelebracion,
    tipocontratoid:tipocontratoid,idempresa:idempresa,representante_legal:representante_legal,codigoactividadid:codigoactividadid,
    empresaregion:empresaregion,empresacomuna:empresacomuna,calle:calle,numero:numero,idtrabajador:idtrabajador,nacionalidad:nacionalidad,
    correotrabajador:correotrabajador,telefonotrabajador:telefonotrabajador,trabajadorregion:trabajadorregion,trabajadorcomuna:trabajadorcomuna,
    calletrabajador:calletrabajador,numerotrabajador:numerotrabajador,departamentotrabajador:departamentotrabajador,centrocosto:centrocosto,Charge:Charge,
    ChargeDescripcion:ChargeDescripcion,regionespecifica:regionespecifica,comunaespecifica:comunaespecifica,calleespecifica:calleespecifica,
    numeroespecifica:numeroespecifica,subcontratacionrut:subcontratacionrut,subcontratacionrazonsocial:subcontratacionrazonsocial,
    transitoriosrut:transitoriosrut,transitoriosrazonsocial:transitoriosrazonsocial,tiposueldo:tiposueldo,sueldo:sueldo,formapago:formapago,
    periodopagogra:periodopagogra,detallerenumeraciongra:detallerenumeraciongra,periodopagot:periodopagot,fechapagot:fechapagot,formapagot:formapagot,
    anticipot:anticipot,noresolucion:noresolucion,exfecha:exfecha,tipojornada:tipojornada,horaspactadas:horaspactadas,dias:dias,colacion:colacion,
    colacionimp:colacionimp,fecha_inicio:fecha_inicio,fecha_termino:fecha_termino,estipulacion1:estipulacion1,estipulacion2:estipulacion2,
    estipulacion3:estipulacion3,estipulacion4:estipulacion4,estipulacion5:estipulacion5,estipulacion6:estipulacion6,estipulacion7:estipulacion7,
    estipulacion8:estipulacion8,estipulacion9:estipulacion9,estipulacion10:estipulacion10,estipulacion11:estipulacion11,estipulacion12:estipulacion12,
    ...
    $reg = $c->buscarregion($regioncelebracion);
    $regioncelebracion = $reg->getNombre();
    $comunacelebracion = $_POST['comunacelebracion'];
    $comcel = $c->buscarcomuna($comunacelebracion);
    $comunacelebracion = $comcel->getNombre();
    $fechacelebracion = $_POST['fechacelebracion'];

    //Informacion Empresa
    $representante_legal = $_POST['representante_legal'];
    $rep = $c->BuscarRepresentanteLegal($representante_legal);
    $representante_legal = $rep->getNombre() . " " . $rep->getApellido1() . " " . $rep->getApellido2();
    $rutrepresentantelegal = $rep->getRut();
    ...
    //Informacion Empresa
    $nacionalidadid = $_POST['nacionalidad'];
    ...
    $reges = $c->buscarregion($regionespecifica);
    $regionespecifica = $reges->getNombre();
    $comunaespecifica = $_POST['comunaespecifica'];
    $comes = $c->buscarcomuna($comunaespecifica);
    $comunaespecifica = $comes->getNombre();
    ...
    if ($result == true) {
        ...
        $eventos = "Se Registro un nuevo contrato de trabajo para el trabajador " . $tra->getNombre() . " " . $tra->getApellido1() . " " . $tra->getApellido2();
        $c->RegistrarAuditoriaEventos($usuario, $eventos);
    } else {
        echo "Error al registrar el contrato de trabajo";
    }
}
